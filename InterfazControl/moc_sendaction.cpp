/****************************************************************************
** Meta object code from reading C++ file 'sendaction.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.1.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "sendaction.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'sendaction.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.1.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_SendAction_t {
    QByteArrayData data[12];
    char stringdata[124];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_SendAction_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_SendAction_t qt_meta_stringdata_SendAction = {
    {
QT_MOC_LITERAL(0, 0, 10),
QT_MOC_LITERAL(1, 11, 11),
QT_MOC_LITERAL(2, 23, 0),
QT_MOC_LITERAL(3, 24, 5),
QT_MOC_LITERAL(4, 30, 14),
QT_MOC_LITERAL(5, 45, 9),
QT_MOC_LITERAL(6, 55, 9),
QT_MOC_LITERAL(7, 65, 4),
QT_MOC_LITERAL(8, 70, 11),
QT_MOC_LITERAL(9, 82, 6),
QT_MOC_LITERAL(10, 89, 27),
QT_MOC_LITERAL(11, 117, 5)
    },
    "SendAction\0changeDepth\0\0value\0"
    "takeScreenshot\0saveVideo\0axisEvent\0"
    "axis\0buttonEvent\0button\0"
    "QGameControllerButtonEvent*\0event\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_SendAction[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   39,    2, 0x05,
       4,    0,   42,    2, 0x05,
       5,    0,   43,    2, 0x05,

 // slots: name, argc, parameters, tag, flags
       6,    2,   44,    2, 0x08,
       8,    2,   49,    2, 0x08,

 // signals: parameters
    QMetaType::Void, QMetaType::Double,    3,
    QMetaType::Void,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void, QMetaType::QString, QMetaType::Int,    7,    3,
    QMetaType::Void, QMetaType::QString, 0x80000000 | 10,    9,   11,

       0        // eod
};

void SendAction::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        SendAction *_t = static_cast<SendAction *>(_o);
        switch (_id) {
        case 0: _t->changeDepth((*reinterpret_cast< double(*)>(_a[1]))); break;
        case 1: _t->takeScreenshot(); break;
        case 2: _t->saveVideo(); break;
        case 3: _t->axisEvent((*reinterpret_cast< QString(*)>(_a[1])),(*reinterpret_cast< int(*)>(_a[2]))); break;
        case 4: _t->buttonEvent((*reinterpret_cast< QString(*)>(_a[1])),(*reinterpret_cast< QGameControllerButtonEvent*(*)>(_a[2]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (SendAction::*_t)(double );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&SendAction::changeDepth)) {
                *result = 0;
            }
        }
        {
            typedef void (SendAction::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&SendAction::takeScreenshot)) {
                *result = 1;
            }
        }
        {
            typedef void (SendAction::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&SendAction::saveVideo)) {
                *result = 2;
            }
        }
    }
}

const QMetaObject SendAction::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_SendAction.data,
      qt_meta_data_SendAction,  qt_static_metacall, 0, 0}
};


const QMetaObject *SendAction::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *SendAction::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_SendAction.stringdata))
        return static_cast<void*>(const_cast< SendAction*>(this));
    return QWidget::qt_metacast(_clname);
}

int SendAction::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 5)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void SendAction::changeDepth(double _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void SendAction::takeScreenshot()
{
    QMetaObject::activate(this, &staticMetaObject, 1, 0);
}

// SIGNAL 2
void SendAction::saveVideo()
{
    QMetaObject::activate(this, &staticMetaObject, 2, 0);
}
QT_END_MOC_NAMESPACE
